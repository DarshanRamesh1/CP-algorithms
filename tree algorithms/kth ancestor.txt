...
void dfs(int root, vector<vector<int>>& edges, vector<bool>& visited,vector<vector<int>>& parent) {
    visited[root] = true;
    for (auto i : edges[root]) 
    {
        ...
        {
            parent[i][0] = root;
            dfs(i, edges, visited, parent);
        }
    }
}

int kthParent(int x, int k, int n, vector<vector<int>>& parent) 
{
...
int main() 
{
    ...
    while(t--)
    {
        ...
        while(q--)
        {
            ...
            cout<<kthParent(x,k,n,parent)<<nline;
        }
    }   
    return 0;
}  
